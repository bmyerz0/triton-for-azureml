...
def benchmark(N, provider):
    ...
    if provider == 'torch-manual-jit':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: softmax_torch_opt(x))#, quantiles=quantiles)
    if provider == 'torch-manual':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: softmax_torch(x))#, quantiles=quantiles)
